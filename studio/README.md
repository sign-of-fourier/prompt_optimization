...
Point-and-click front end for [bpto](https://github.com/sign-of-fourier/bpto): draw a program (prompt steps wired
into a graph, orchestrating steps that choose the next step), attach a labelled dataset, validate it, run GEPA or BO
in the expand seat, watch the tree. Served at `promptcompression.ai/studio/`.

```
app/          FastAPI backend (routes are unprefixed; nginx mounts them at /studio/api/)
...
web/          React + React Flow canvas (vite); `npm run build` -> web/dist, served by nginx
deploy/       nginx location block, systemd unit
