...
// Each card: anchor = data-tut selector to spotlight; tab = tab to switch to; check(spec, ctx) -> done (action steps);
// action = { label, run(ctx) } does the step for the user. Steps without check are informational.
export const STEPS = [
  { title: 'Welcome to the studio', body: 'You will build a small program - one or more prompts wired together - attach a labelled dataset, and let the optimizer rewrite the prompts until they score better. The example is support-ticket triage: a message comes in, one prompt summarizes it, a second prompt picks the queue. Twelve short cards; skip any time. (Prefer to start from the finished thing? The Library has it as a template.)' },
  { title: 'Drag a Prompt onto the canvas', anchor: 'palette-module', tab: 'build',
    body: 'A Prompt is one node the optimizer rewrites. Press on it in the palette and drop it anywhere on the canvas.',
  ...
  { title: 'Click it and write the prompt', anchor: 'side-template', tab: 'build',
    body: 'Click the prompt to open it on the right. Words in {braces} are placeholders; they are filled from your dataset\'s columns. Rename it "summarize" and use this template:\n\n' + EXAMPLE.summarize,
  ...
  { title: 'Add an output field', anchor: 'side-fields', tab: 'build',
    body: 'Prompts return structured fields rather than loose text, so the next prompt and the scorer can pick one out. Add a field named "summary".',
  ...
  { title: 'Drag a second Prompt', anchor: 'palette-module', tab: 'build',
    body: 'This one will route the ticket. Drop it to the right of the first. Name it "route", give it an output field "queue", and use this template:\n\n' + EXAMPLE.route,
  ...
  { title: 'Connect the two prompts', anchor: 'canvas', tab: 'build',
    body: 'Each prompt has a dot on its left (input) and right (output). Press on the first prompt\'s RIGHT dot, drag, and release on the second prompt\'s LEFT dot. A solid edge with an arrow appears.\n\nThe dashed lines you saw go to Evaluate: they only mean "this prompt currently ends the program". They appear and disappear on their own; solid edges are yours.',
    check: s => s.edges.length >= 1, action: { label: 'Connect for me', run: ctx => ctx.connect(0, 1) } },
  { title: 'Map what the second prompt receives', anchor: 'edge-mapping', tab: 'build',
    body: 'Click the edge. The route prompt has a {summary} placeholder; map it to the summarize prompt\'s "summary" field. Every prompt can also read the dataset columns directly, so {message} would work in either one.',
    check: s => s.edges.some(e => Object.keys(e.mapping).length > 0), action: { label: 'Map it for me', run: ctx => ctx.mapEdge(0, { summary: 'summary' }) } },
  { title: 'Routers (for later)', anchor: 'palette-orchestrate', tab: 'build',
    body: 'A prompt with two or more outgoing edges is a router: the model returns a "next" field naming the edge to take, one edge is the default if that cannot be parsed, and loops back to earlier prompts are allowed, capped by "max prompt calls per example". Not needed for this example.' },
  { title: 'Tell Evaluate what to grade', anchor: 'evaluate-node', tab: 'build',
    body: 'The prompt with no outgoing edge is what gets graded, here "route". Click the Evaluate block and set the scorer to compare the output field "queue" with the label. The objective weighs the metrics; accuracy alone is fine here.',
  ...
  { title: 'The optimizer', anchor: 'btn-optimizer', tab: 'build',
    body: 'The loop is fixed: pick a prompt from the current best set, show a reflection model a few of its failures, rewrite one prompt, and keep the rewrite only if it beats its parent on the same rows. The gear opens its knobs: rounds, budget, the check batch, and whether the next prompt to rewrite is sampled (Explore) or chosen by a model of past results (Guided). Your tier caps how many run in parallel. Two rounds is enough for the tutorial.' },
